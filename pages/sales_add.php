...
include('../dist/includes/dbcon.php');
	...
	$query=mysqli_query($con,"select * from temp_trans where branch_id='$branch'")or die(mysqli_error($con));
		while ($row=mysqli_fetch_array($query))
		{
			...
			$cntqty = 0;
			$query2=mysqli_query($con,"select * from product where prod_id='$pid' and branch_id='$branch'")or die(mysqli_error($con));
			while ($row=mysqli_fetch_array($query2))
			{
				$cntqty=$row['prod_qty'];
			}
			mysqli_query($con,"INSERT INTO producttracker(Action, Saleqty, Balance, prod_id,  branch_id) 
			VALUES('SALE','$qty','$cntqty','$pid','$branch')")or die(mysqli_error($con));
